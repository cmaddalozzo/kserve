...
s3_transfer_config = TransferConfig(
    multipart_threshold=int(os.environ.get("S3_MULTIPART_THRESHOLD", 8 * 1024 * 1024)),
    multipart_chunksize=int(os.environ.get("S3_MULTIPART_CHUNKSIZE", 8 * 1024 * 1024)),
...
class Storage(object):  # pylint: disable=too-few-public-methods
    @staticmethod
    def download(uri: str, out_dir: str = None, concurrency: int = 1) -> str:
        ...
        if uri.startswith(_GCS_PREFIX):
            ...
            Storage._download_s3(uri, out_dir, concurrency)
        ...
        logger.info(f"Successfully copied {uri} to {out_dir} in {duration:.3f}s")
        return out_dir
    ...
    @staticmethod
    def _download_s3(uri, temp_dir: str, concurrency: int):
        ...
        for obj in bucket.objects.filter(Prefix=bucket_path):
            ...
            target = _temp_dir / target_key
            if not os.path.exists(os.path.dirname(target)):
                os.makedirs(os.path.dirname(target), exist_ok=True)

            files_to_download.append((obj.key, target))

            # If the exact object is found, then it is sufficient to download that and break the loop
            if exact_obj_found:
                break

        def download_file(key: str, target: Path) -> Path:
            logger.info(f"Downloading file: {key}")
            start = time.time()
            bucket.download_file(key, str(target), Config=s3_transfer_config)
            duration = time.time() - start
            logger.info(f"Downloaded file {target} in {duration:.4f}s")
            return target

        if concurrency == 1 or len(files_to_download) == 1:
            for key, target in files_to_download:
                downloaded_files.append(download_file(key, target))
        else:
            with ThreadPoolExecutor(max_workers=concurrency) as executor:
                future_to_key = {
                    executor.submit(download_file, f[0], f[1]): f
        ...
        # Unpack compressed file, supports .tgz, tar.gz and zip file formats.
            ...
            mimetype, _ = mimetypes.guess_type(downloaded_files[0])
            if mimetype in ["application/x-tar", "application/zip"]:
                Storage._unpack_archive_file(downloaded_files[0], mimetype, temp_dir)
